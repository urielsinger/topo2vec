...
def get_dataset_as_tensor(dataset: Dataset) -> Tuple[Tensor, Tensor]:
    ...
    return get_random_part_of_dataset(dataset, dataset_length)


def get_random_part_of_dataset(dataset: Dataset, size_wanted: int) -> Tuple[Tensor, Tensor]:
    '''
    Args:
        dataset:
        size_wanted: number of rows wanted in the generated Tensor

    Returns: a tuple: (the images tensor, the classifications tensor)

    '''

    data_loader = DataLoader(dataset, shuffle=True, num_workers=0, batch_size=size_wanted)
