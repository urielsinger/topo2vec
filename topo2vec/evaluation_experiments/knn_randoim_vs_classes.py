...
train_set_size_for_knn_max = 5
train_set_size_for_knn_min = 4
class_paths_special, class_names_special = get_paths_and_names_wanted(
    special_classes_for_validation, CLASS_PATHS_SPECIAL, CLASS_NAMES_SPECIAL)
...
for special_class_path, special_class_name in zip(class_paths_special, class_names_special):
    knn_special_test_dataset = OneVsRandomDataset(original_radiis, test_set_size_for_knn, VALIDATION_HALF, special_class_path, random_seed=665)
    knn_special_test_dataset_resnet = OneVsRandomDataset(original_radiis, test_set_size_for_knn, VALIDATION_HALF, special_class_path, radii=RESNET_RADII, random_seed=665)
    ...
    amphib_ae_accuracy = []
    number_of_examples = list(range(train_set_size_for_knn_min, train_set_size_for_knn_max))
    for train_set_size_for_knn in tqdm.tqdm(number_of_examples):
        ...
        for seed in tqdm.tqdm(seeds):
            logging.info(seed)
            knn_special_train_dataset = OneVsRandomDataset(original_radiis, train_set_size_for_knn, TRAIN_HALF, special_class_path, random_seed=seed)
            ...
            with torch.no_grad():
                knn_classifier_special_classes_test_log_dict_classic = \
                    knn_classifier_test(classic_model_best, knn_special_train_dataset, knn_special_train_dataset, knn_special_test_dataset, 'special')

            with torch.no_grad():
                knn_classifier_special_classes_test_log_dict_resnet = \
                    knn_classifier_test(topo_resnet_model, knn_special_train_dataset_resnet, knn_special_train_dataset_resnet,
                                        knn_special_test_dataset_resnet, 'special')

            with torch.no_grad():
                knn_classifier_special_classes_test_log_dict_resnet_transfer = \
                    knn_classifier_test(topo_resnet_full, knn_special_train_dataset_resnet, knn_special_train_dataset_resnet,
                                        knn_special_test_dataset_resnet, 'special')

            with torch.no_grad():
                knn_classifier_special_classes_test_log_dict_amphib_autoencoder = \
                    knn_classifier_test(amphib_autoencoder, knn_special_train_dataset, knn_special_train_dataset,
                                        knn_special_test_dataset, 'special')

            classic_list.append(knn_classifier_special_classes_test_log_dict_classic[f'knn_test_special_{what_to_plot}'])
            res_list.append(knn_classifier_special_classes_test_log_dict_resnet[f'knn_test_special_{what_to_plot}'])
            res_full_list.append(knn_classifier_special_classes_test_log_dict_resnet_transfer[f'knn_test_special_{what_to_plot}'])
            amphib_ae_list.append(knn_classifier_special_classes_test_log_dict_amphib_autoencoder[f'knn_test_special_{what_to_plot}'])
        ...
        logging.info(knn_classifier_special_classes_test_log_dict_resnet)
        resnet_accuracy.append(np.mean(res_list))
        logging.info(res_list)

        logging.info('results on the resnet transfer latent space')
        logging.info(knn_classifier_special_classes_test_log_dict_resnet_transfer)
        ...
        logging.info(knn_classifier_special_classes_test_log_dict_amphib_autoencoder)
    ...
    plt.legend()
    plt.title(f'knn - few shot: {special_class_name} vs. random')
    plt.xlabel('number of poositive examples')
    plt.ylabel(f'{what_to_plot}')
    plt.savefig(f'results_evaluation_experiments\\{special_class_name}_{what_to_plot}_knn_one.png')
    plt.clf()